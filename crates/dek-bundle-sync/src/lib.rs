...
impl BundleSyncAgent {
    pub fn new(cloud_url: &str, device_id: &str, mtls: &MtlsConfig) -> Result<Self> {
    ...
    pub async fn run_pipeline(&self) -> Result<DekConfig> {
        ...
        if !res.status().is_success() {
            error!("[BundleSync] Failed to fetch device config: HTTP {}", res.status());
            return Err(anyhow::anyhow!("Config fetch failed"));
        }
        let dek_config: DekConfig = res.json().await.context("Failed to parse DekConfig")?;
        info!("[BundleSync] Successfully fetched device config for tenant: {}", dek_config.tenant_id);
        ...
        let bundle_payload = if res.status().is_success() {
            ...
            info!("[BundleSync] Discovered new bundle: {} v{}", artifact.id, artifact.version);
            ...
            let verifying_key = VerifyingKey::from_bytes(
                public_key_bytes.as_slice().try_into().context("Invalid public key length")?,
            )?;
            let signature = Signature::from_bytes(
                signature_bytes.as_slice().try_into().context("Invalid signature length")?,
            );

            let payload_string = serde_json::to_string(&bundle_info.payload)?;

            if verifying_key.verify(payload_string.as_bytes(), &signature).is_ok() {
                artifact.state = ArtifactState::SignatureVerified;
                info!("[BundleSync] State: {:?} - Signature valid!", artifact.state);
        ...
        if let Some(spire_config) = &dek_config.spire_server {
            match dek_spire_node::SpireNodeAgent::new(&spire_config.endpoint, &dek_config.mtls) {
                Ok(agent) => {
                    match agent.attest_and_fetch_svid(&dek_config.device_id).await {
                        Ok(spiffe_id) => {
        ...
        info!("[BundleSync] State: {:?} - Staged combined config atomically to {:?}", ArtifactState::Staged, active_bundle_path);

        // Activation is implicit via dek-mcp-proxy's notify watcher
        info!("[BundleSync] State: {:?} - Pipeline complete. Proxy will activate implicitly.", ArtifactState::Active);
