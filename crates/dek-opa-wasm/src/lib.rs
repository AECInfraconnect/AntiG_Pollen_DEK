...
impl OpaWasmAdapter {
    pub fn new(wasm_path: &str, profile: Option<WasmProfile>) -> anyhow::Result<Self> {
        ...
        // Map OPA builtins
        linker.func_wrap("env", "opa_abort", |mut caller: Caller<'_, RuntimeState>, addr: i32| {
        ...
        linker.func_wrap("env", "opa_println", |mut caller: Caller<'_, RuntimeState>, addr: i32| {
            let mem = caller.get_export("memory").unwrap().into_memory().unwrap();
            let data = mem.data(&caller);
            let mut end = addr as usize;
            while end < data.len() && data[end] != 0 {
        ...
        linker.func_wrap("env", "opa_builtin0", |_caller: Caller<'_, RuntimeState>, _id: i32, _ctx: i32| -> i32 { 0 })?;
        linker.func_wrap("env", "opa_builtin1", |_caller: Caller<'_, RuntimeState>, _id: i32, _ctx: i32, _1: i32| -> i32 { 0 })?;
        linker.func_wrap("env", "opa_builtin2", |_caller: Caller<'_, RuntimeState>, _id: i32, _ctx: i32, _1: i32, _2: i32| -> i32 { 0 })?;
        linker.func_wrap("env", "opa_builtin3", |_caller: Caller<'_, RuntimeState>, _id: i32, _ctx: i32, _1: i32, _2: i32, _3: i32| -> i32 { 0 })?;
        linker.func_wrap("env", "opa_builtin4", |_caller: Caller<'_, RuntimeState>, _id: i32, _ctx: i32, _1: i32, _2: i32, _3: i32, _4: i32| -> i32 { 0 })?;
    ...
    fn write_json_to_memory(&self, store: &mut Store<RuntimeState>, instance: &Instance, json_str: &str) -> anyhow::Result<i32> {
        let malloc = instance.get_typed_func::<i32, i32>(store.as_context_mut(), "opa_malloc")?;
        let json_parse = instance.get_typed_func::<(i32, i32), i32>(store.as_context_mut(), "opa_json_parse")?;
    ...
    fn read_json_from_memory(&self, store: &mut Store<RuntimeState>, instance: &Instance, addr: i32) -> anyhow::Result<String> {
        let json_dump = instance.get_typed_func::<i32, i32>(store.as_context_mut(), "opa_json_dump")?;
...
impl PolicyEvaluator for OpaWasmAdapter {
    ...
    async fn evaluate(&self, input: EvalRequest) -> PluginResult<PolicyDecision> {
        ...
        let eval_ctx_new = instance.get_typed_func::<(), i32>(&mut store, "opa_eval_ctx_new")
            .map_err(|e| PluginError::Execution(format!("missing opa_eval_ctx_new: {e}")))?;
        let ctx = eval_ctx_new.call(&mut store, ())
            .map_err(|e| PluginError::Execution(format!("opa_eval_ctx_new failed: {e}")))?;

        // 2. Write input
        let input_addr = self.write_json_to_memory(&mut store, &instance, &input_str)
            .map_err(|e| PluginError::Execution(format!("write input error: {e}")))?;

        let eval_ctx_set_input = instance.get_typed_func::<(i32, i32), ()>(&mut store, "opa_eval_ctx_set_input")
            .map_err(|e| PluginError::Execution(format!("missing opa_eval_ctx_set_input: {e}")))?;
        eval_ctx_set_input.call(&mut store, (ctx, input_addr))
            .map_err(|e| PluginError::Execution(format!("opa_eval_ctx_set_input failed: {e}")))?;

        // 3. Evaluate
        let eval = instance.get_typed_func::<i32, ()>(&mut store, "opa_eval")
        ...
        // 4. Get result
        let eval_ctx_get_result = instance.get_typed_func::<i32, i32>(&mut store, "opa_eval_ctx_get_result")
            .map_err(|e| PluginError::Execution(format!("missing opa_eval_ctx_get_result: {e}")))?;
        let result_addr = eval_ctx_get_result.call(&mut store, ctx)
            .map_err(|e| PluginError::Execution(format!("opa_eval_ctx_get_result failed: {e}")))?;

        if result_addr != 0 {
            let res_json = self.read_json_from_memory(&mut store, &instance, result_addr)
            ...
            if let Ok(output_val) = serde_json::from_str::<serde_json::Value>(&res_json) {
                ...
                if let Some(arr) = output_val.as_array() {
                    ...
                    allow = output_val.get("allow").and_then(|v| v.as_bool()).unwrap_or(false);
                }
            } else {
