use dek_agent_discovery::model::{DiscoveredAgentCandidateV2, ControlBindingKind};
use crate::planner::{ControlLevel, LocalCapabilitySnapshot, PolicyFeasibilityResult, DomainFeasibility, ControlDomain, Policy, MethodStatus};
...
pub fn assess(
    ...
    for binding in &candidate.suggested_control_bindings {
        let domain = match binding.kind {
            ControlBindingKind::McpStdioWrapper | ControlBindingKind::McpHttpProxy | ControlBindingKind::OpenAiCompatibleProxy | ControlBindingKind::AnthropicCompatibleProxy | ControlBindingKind::OllamaProxy => Some(ControlDomain::McpTool),
