...
pub fn is_newer(current_version: &str, latest_tag: &str) -> Result<bool> {
    ...
    let latest_ver = match Version::parse(latest) {
        Ok(v) => v,
        Err(_) => {
            eprintln!("Failed to parse remote version {}; acting conservative", latest);
            return Ok(false);
        }
    };
...
pub fn download_update(release: &GitHubRelease, temp_dir: &Path) -> Result<(PathBuf, PathBuf, PathBuf, PathBuf)> {
    ...
    for asset in &release.assets {
        if asset.name.contains(platform) && !asset.name.ends_with(".sig") && !asset.name.ends_with(".pem") {
    ...
    let archive_url = archive_url.context(format!("Could not find archive asset for platform {}", platform))?;
...
fn download_file(url: &str, dir: &Path, filename: &str) -> Result<PathBuf> {
    let resp = ureq::get(url).call().context(format!("Failed to download {}", url))?;
...
pub fn verify_sha256(archive_path: &Path, sum_path: &Path) -> Result<()> {
    ...
    let expected_hash = match expected_hash {
        ...
        None => {
            ...
            let platform_str = if cfg!(windows) { "windows-amd64" } else if cfg!(target_os = "macos") { "macos-amd64" } else { "linux-amd64" };
            let mut found = None;
            for line in sums.lines() {
    ...
    if actual_hash != expected_hash {
        anyhow::bail!("SHA256 mismatch! Expected {}, got {}", expected_hash, actual_hash);
    }
...
pub fn verify_all(archive_path: &Path, sum_path: &Path, sig_path: &Path, pem_path: &Path) -> Result<()> {
