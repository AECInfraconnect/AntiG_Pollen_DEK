...
pub async fn run_preflight_tests(
    config: &DekConfig,
        ...
        match router.authorize(test.input.clone()).await {
            Ok(decision) => {
                if decision.decision != test.expected_decision {
                    let msg = format!(
                        "Preflight test '{}' failed: expected {}, got {}",
                        test.name, test.expected_decision, decision.decision
                    );
                    error!("{}", msg);
            ...
            Err(e) => {
                let msg = format!(
                    "Preflight test '{}' failed with execution error: {}",
                    test.name, e
                );
                error!("{}", msg);
                return Err(ActivationError::PreflightFailed(msg));
