...
use crate::model::{TokenUsage, AgentObservationEvent, EventKind};
...
pub fn parse_llm_usage(host: &str, body: &serde_json::Value) -> Option<(String, TokenUsage)> {
    ...
    let usage = body.get("usage").or_else(|| {
        body.get("message_delta").and_then(|m| m.get("usage"))
    })?;

    // OpenAI/Ollama/vLLM/NIM: prompt_tokens/completion_tokens
    let (input, output) = if let Some(p) = usage.get("prompt_tokens") {
        (p.as_i64(), usage.get("completion_tokens").and_then(|v| v.as_i64()))
    // Anthropic: input_tokens/output_tokens
    } else if let Some(i) = usage.get("input_tokens") {
        (i.as_i64(), usage.get("output_tokens").and_then(|v| v.as_i64()))
    } else { (None, None) };

    Some((provider, TokenUsage {
        input_tokens: input,
...
pub fn llm_call_event(tenant: &str, trace_id: &str, agent_id: Option<String>,
                      host: &str, body: &serde_json::Value, latency_ms: i64)
