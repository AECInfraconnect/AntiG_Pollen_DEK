...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/v1/tenants/:tenant_id/devices/:device_id/telemetry/security-events", post(ingest_security_events))
        .route("/v1/tenants/:tenant_id/devices/:device_id/telemetry/runtime-metrics", post(ingest_runtime_metrics))
        .route("/v1/tenants/:tenant_id/devices/:device_id/telemetry/ebpf-events", post(ingest_ebpf_events))
}
...
async fn ingest_security_events(Path((tenant_id, device_id)): Path<(String, String)>, State(state): State<AppState>, Json(payload): Json<Value>) -> impl IntoResponse {
    if verify_device_tenant(&state, &tenant_id, &device_id).is_err() {
        return (StatusCode::FORBIDDEN, Json(json!({"error": "tenant mismatch"})));
    }
    info!("CLOUD RECEIVED SECURITY EVENTS from {}/{}: {}", tenant_id, device_id, payload);
    (StatusCode::OK, Json(json!({ "status": "ingested" })))
}

async fn ingest_runtime_metrics(Path((tenant_id, device_id)): Path<(String, String)>, State(state): State<AppState>, Json(payload): Json<Value>) -> impl IntoResponse {
    if verify_device_tenant(&state, &tenant_id, &device_id).is_err() {
        return (StatusCode::FORBIDDEN, Json(json!({"error": "tenant mismatch"})));
    }
    info!("CLOUD RECEIVED RUNTIME METRICS from {}/{}: {}", tenant_id, device_id, payload);
    (StatusCode::OK, Json(json!({ "status": "ingested" })))
}

async fn ingest_ebpf_events(Path((tenant_id, device_id)): Path<(String, String)>, State(state): State<AppState>, Json(payload): Json<Value>) -> impl IntoResponse {
    if verify_device_tenant(&state, &tenant_id, &device_id).is_err() {
        return (StatusCode::FORBIDDEN, Json(json!({"error": "tenant mismatch"})));
    }
    info!("CLOUD RECEIVED EBPF EVENTS from {}/{}: {}", tenant_id, device_id, payload);
    (StatusCode::OK, Json(json!({ "status": "ingested" })))
}
