        ...
        if let Some(servers) = json.get("mcpServers").and_then(|v| v.as_object()) {
            for (name, cfg) in servers {
                ...
                let url = cfg
                    .get("url")
                    .and_then(|v| v.as_str())
                    .or_else(|| cfg.get("endpoint").and_then(|v| v.as_str()));

                out.push(McpServerConfigEvidence {
                    config_path_hash: crate::redaction::sha256_string(&p.to_string_lossy()),
                    config_path_redacted: crate::redaction::redact_path_for_ui(
                        &p.to_string_lossy(),
                    ),
                    ...
                    command_template: command.map(|c| {
