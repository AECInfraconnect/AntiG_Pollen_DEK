...
impl FingerprintService {
    ...
    pub async fn update_from_bytes(&self, raw: &[u8], sig_b64: &str) -> anyhow::Result<u64> {
        ...
        let mut candidate = {
            ...
            FingerprintDb {
                ...
                collapse_rules: cur.collapse_rules.clone(),
            }
        };
        ...
        let new_version = candidate.version;
        {
            ...
            if let Ok(mut prev_w) = self.prev_db.write() {
                *prev_w = Some(FingerprintDb {
                    ...
                    collapse_rules: w.collapse_rules.clone(),
                });
            }
    ...
    pub async fn update_with_fallback<F, Fut>(
        &self,
        raw: &[u8],
        sig_b64: &str,
        fetch_full: F,
    ...
    pub fn rollback(&self) -> anyhow::Result<u64> {
        ...
        if let Some(prev) = prev_w.take() {
            ...
            *w = prev;
            tracing::info!(
                "Rolled back fingerprint definition to version {}",
                rb_version
            );
            Ok(rb_version)
        } else {
    ...
    pub fn snapshot(&self) -> Vec<AgentSignatureV2> {
        self.db
            .read()
            .map(|d| d.by_id.values().cloned().collect())
            .unwrap_or_default()
    }
}
