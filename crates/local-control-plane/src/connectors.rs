...
async fn set_override(
    ...
    tracing::info!("Setting override for PDP {} to state {} with delay {:?}", id, payload.forced_state, payload.auto_recovery_delay);
...
async fn test_connection(
    ...
    let cfg = match st.connector_store.get(&_tenant, &id).await {
        Ok(Some(c)) => match serde_json::from_value::<ConnectorConfig>(c) {
            Ok(c) => c,
            Err(_) => return Json(TestResult{ ok:false, latency_ms:0, detail:"invalid config".into() }),
        },
...
async fn list(
    ...
    let list = st.connector_store.list(&tenant).await.map_err(ApiError::Internal)?;
    let mut configs = vec![];
    for val in list {
...
async fn upsert(
    ...
    st.connector_store.upsert(&tenant, &payload.id, &val).await.map_err(ApiError::Internal)?;
    Ok(Json(payload))
}
