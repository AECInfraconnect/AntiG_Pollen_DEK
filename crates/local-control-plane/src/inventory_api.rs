...
use dek_agent_observer::aggregate::{
    aggregate_resources, aggregate_tools, ObservedResource, ObservedTool,
};
...
async fn list_resources(
    ...
    for ev in events_json {
        if let Ok(p) = serde_json::from_value::<ResourceAccessPayload>(ev) {
...
async fn list_tools(
    ...
    let mut payloads = Vec::new();
    for ev in events_json {
        if let Ok(p) = serde_json::from_value::<ToolUsagePayload>(ev) {
            if let Some(agent_id) = &query.agent_id {
                if p.agent_id != *agent_id {
                    continue;
                }
        ...
        .await
    {
        for ev in evs {
            if let Some(tid) = &query.tool_id {
                let mut matched = false;
                if let Some(t) = ev.pointer("/details/tool_id").and_then(|v| v.as_str()) {
                    if *tid == t {
                        matched = true;
                    }
