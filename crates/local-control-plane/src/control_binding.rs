...
pub async fn do_apply_binding(binding_id: &str) -> Result<(), String> {
    let mock_config_dir = std::env::temp_dir()
        .join(".pollen_dek")
        .join("mcp_configs");
    ...
    if !original_file.exists() {
        ...
        let _ = fs::write(&original_file, serde_json::to_string_pretty(&initial_config).unwrap());
    }
    ...
    if let Ok(content) = fs::read_to_string(&original_file) {
        ...
        if let Ok(mut config) = serde_json::from_str::<serde_json::Value>(&content) {
            if let Some(servers) = config.get_mut("mcpServers").and_then(|v| v.as_object_mut()) {
                for (_name, server) in servers.iter_mut() {
                    if let Some(obj) = server.as_object_mut() {
                        let original_command = obj.get("command").and_then(|v| v.as_str()).unwrap_or("").to_string();
                        let original_args = obj.get("args").and_then(|v| v.as_array()).cloned().unwrap_or_default();
                        
                        obj.insert("command".to_string(), serde_json::json!("dek-stdio-wrapper"));
            ...
            let _ = fs::write(&original_file, serde_json::to_string_pretty(&config).unwrap());
        }
    }
    Ok(())
}
