# คู่มือการแก้ไขปัญหาเบื้องต้น (Troubleshooting) - Pollen DEK
...
**อาการ**: คำสั่ง `pollen-dekctl enroll` ค้าง หรือแสดงข้อผิดพลาดเกี่ยวกับการเชื่อมต่อ (Connection error)
...
**วิธีแก้**: ตรวจสอบว่าอุปกรณ์ได้รับการลงทะเบียนอย่างถูกต้องแล้ว และมีไฟล์ `bootstrap.json` ใน `~/.pollen/dek/` หากคุณกำลังทดสอบสถานการณ์จำลอง (Chaos testing) ให้ตรวจสอบว่าคุณไม่ได้เผลอจำลองเหตุการณ์ Cloud Outage ไว้ใน Mock-Cloud
...
**วิธีแก้**: ตรวจสอบให้แน่ใจว่า DEK ถูกรันด้วยสิทธิ์ root (`CAP_BPF` และ `CAP_NET_ADMIN`) หรือใช้คำสั่ง `dmesg` หรือ `journalctl -u pollen-dek` เพื่อตรวจสอบว่ามีข้อผิดพลาดเกี่ยวกับ BPF Verifier เกิดขึ้นหรือไม่
