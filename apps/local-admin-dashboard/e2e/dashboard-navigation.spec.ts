...
test.describe("Dashboard Navigation", () => {
  ...
  test("should render sidebar and navigate to all main sections", async ({
    page,
  }) => {
    // 1. Dashboard
    await expect(
    ...
    await expect(
      page
        .getByRole("heading", { name: /policy presets/i, exact: false })
        .first(),
    ).toBeVisible();

    await page.getByRole("link", { name: /policy suggestions/i }).click();
    await expect(
      page.getByRole("heading", { name: /suggestions/i, exact: false }).first(),
    ).toBeVisible();
    ...
    await expect(
      page
        .getByRole("heading", {
          name: /active policies|policy enforcer/i,
          exact: false,
        })
        .first(),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("heading", { name: /alerts/i, exact: false }).first(),
    ).toBeVisible();

    await page.getByRole("link", { name: /audit logs/i }).click();
    await expect(
      page
        .getByRole("heading", { name: /decision logs/i, exact: false })
        .first(),
    ).toBeVisible();

    await page.getByRole("link", { name: /cost & tokens/i }).click();
    await expect(
      page.getByRole("heading", { name: /cost ledger/i, exact: false }).first(),
    ).toBeVisible();
    ...
    await expect(
      page
        .getByRole("heading", { name: /identity & network/i, exact: false })
        .first(),
    ).toBeVisible();

    await page.getByRole("link", { name: /data resources/i }).click();
    await expect(
      page.getByRole("heading", { name: /resources/i, exact: false }).first(),
    ).toBeVisible();

    await page.getByRole("link", { name: /simulator/i }).click();
    await expect(
      page.getByRole("heading", { name: /simulator/i, exact: false }).first(),
    ).toBeVisible();

    await page.getByRole("link", { name: /bundles & sync/i }).click();
    await expect(
      page.getByRole("heading", { name: /bundles/i, exact: false }).first(),
    ).toBeVisible();

    await page.getByRole("link", { name: /auto discovery/i }).click();
    await expect(
      page
        .getByRole("heading", { name: /auto discovery/i, exact: false })
        .first(),
    ).toBeVisible();

    await page.getByRole("link", { name: /global settings/i }).click();
    await expect(
      page.getByRole("heading", { name: /settings/i, exact: false }).first(),
    ).toBeVisible();
  });
});
