...
function StepDots({ step, labels }: { step: number; labels: string[] }) {
  return <div className="flex gap-2">{labels.map((l, i) => <span key={i} className={step >= i+1 ? "font-bold text-primary" : "text-muted"}>{l}</span>)}</div>;
}
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return <div className="space-y-4"><h3 className="text-xl font-bold">{title}</h3>{children}</div>;
}
function Toggle({ label, checked, onChange }: { label: string; checked: boolean; onChange: () => void }) {
  return <label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={checked} onChange={onChange} /> {label}</label>;
}
function Radio({ label, checked, onChange }: { label: string; checked: boolean; onChange: () => void }) {
  return <label className="flex items-center gap-2 cursor-pointer"><input type="radio" checked={checked} onChange={onChange} /> {label}</label>;
}
function NextBtn({ disabled, onClick, label }: { disabled: boolean; onClick: () => void; label: string }) {
  return <button disabled={disabled} onClick={onClick} className="mt-4 px-4 py-2 bg-primary text-white rounded disabled:opacity-50">{label}</button>;
}
...
export function SimplePolicyWizard({ agents }: { agents: { id: string; label: string }[] }) {
  ...
  async function toPolicies() {
    ...
    try {
        ...
        setSuggestions([
            { id: "pol_observe", title_en: "Observe All Activity", title_th: "สังเกตการณ์ทุกกิจกรรม", domains: ["network"], recommended_level: "observe" }
        ]);
    }
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <StepDots step={step} labels={[t("step.agent"), t("step.policy"), t("step.level"), t("step.confirm")]} />

      {step === 1 && (
        <Section title={t("step.agent")}>
          {agents.map((a) => (
            <Toggle key={a.id} label={a.label}
              checked={picked.includes(a.id)}
              onChange={() => setPicked((p) => p.includes(a.id) ? p.filter(x=>x!==a.id) : [...p, a.id])} />
          ))}
          <NextBtn disabled={!picked.length || busy} onClick={toPolicies} label={t("common.next")} />
        </Section>
      )}
      ...
      {step === 2 && (
        <Section title={t("step.policy")}>
          {suggestions.map((s) => (
            <Radio key={s.id} label={i18nTitle(s)} checked={policy?.id === s.id}
              onChange={() => { setPolicy(s); setLevel(s.recommended_level); }} />
          ))}
          <NextBtn disabled={!policy} onClick={() => setStep(3)} label={t("common.next")} />
        </Section>
      )}
      ...
      {step === 3 && (
        <Section title={t("step.level")}>
          ...
          <NextBtn disabled={busy} onClick={toConfirm} label={t("common.review")} />
        </Section>
      )}
