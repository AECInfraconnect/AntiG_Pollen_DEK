...
export function SimplePolicyWizard() {
  ...
  const handleNextStep1 = async () => {
    ...
    try {
      ...
      console.error(e);
      setSuggestions([
        { id: "pol_observe", title_en: "Observe All Activity", title_th: "สังเกตการณ์ทุกกิจกรรม", domains: ["network", "file_system"], recommended_level: "observe" },
        { id: "pol_block_unknown", title_en: "Block Unknown Network", title_th: "บล็อกเครือข่ายที่ไม่รู้จัก", domains: ["network"], recommended_level: "enforce" }
      ]);
      setStep(2);
    } finally {
      setLoading(false);
    ...
    <div className="space-y-6 max-w-3xl mx-auto py-8">
      ...
      {step === 1 && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
          <h3 className="text-xl font-semibold">1. {t("step.agent")}</h3>
      ...
      {step === 2 && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
          <h3 className="text-xl font-semibold">2. {t("step.policy")}</h3>
      ...
      {step === 3 && feasibility && !deploySuccess && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
          <h3 className="text-xl font-semibold">3. {t("step.level")}</h3>
          ...
          <div className="flex justify-between pt-6 border-t mt-8">
            ...
            <button
              ...
              className="px-6 py-2 bg-primary text-primary-foreground rounded-md shadow-sm font-medium hover:opacity-90 transition-opacity disabled:opacity-50"
            >
              {loading ? "Deploying..." : t("step.confirm")}
            </button>
          </div>
        </div>
      )}
