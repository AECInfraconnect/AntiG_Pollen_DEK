...
const LEVELS: { id: ControlLevel; icon: string; key: string }[] = [
  { id: "observe", icon: "👁️", key: "control.observe" },
  { id: "warn",    icon: "⚠️", key: "control.warn" },
  { id: "enforce", icon: "🛡️", key: "control.enforce" },
];
    ...
    <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
      {LEVELS.map((l) => (
        <button key={l.id} onClick={() => onChange(l.id)}
          ...
          <div className="mt-1 font-medium">{t(`${l.key}`)}</div>
          <div className="text-xs text-zinc-400">{t(`${l.key}_desc`)}</div>
        </button>
      ))}
    </div>
  );
