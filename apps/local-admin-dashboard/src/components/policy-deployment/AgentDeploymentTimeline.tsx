...
import { DeploymentEvent, EventStatus, DeploymentPhase } from '../../types/deployment';
          ...
          <div key={event.event_id} className="mb-6 ml-6">
            <span
              className={`absolute flex items-center justify-center w-6 h-6 rounded-full -left-3 ring-8 ring-white dark:ring-gray-900 ${
                event.status === EventStatus.Success ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' :
                event.status === EventStatus.Error ? 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300' :
                event.status === EventStatus.Warning ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300' :
