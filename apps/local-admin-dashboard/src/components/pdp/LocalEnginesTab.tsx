...
export function LocalEnginesTab() {
  ...
  const handleAction = async (id: string, actionName: string, actionFn: () => Promise<any>) => {
    ...
    try {
      ...
      setActionStates((prev) => ({ ...prev, [id]: `${actionName} failed: ${e.message || String(e)}` }));
    }
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          Built-in PDP engines managed by DEK. Local engines do not require endpoints.
        </p>
        <button 
          onClick={reload}
              ...
              <tr key={r.id} className="border-b last:border-0 hover:bg-muted/30">
                ...
                <td className="px-4 py-3">
                  <span className={`px-2 py-1 rounded text-xs ${
                    r.status === 'ready' ? 'bg-green-500/10 text-green-500' :
                    r.status === 'error' ? 'bg-red-500/10 text-red-500' :
                    r.status === 'degraded' ? 'bg-yellow-500/10 text-yellow-500' :
                ...
                <td className="px-4 py-3">
                  {r.last_probe ? (
                    <div className={`text-xs ${r.last_probe.ok ? "text-green-500" : "text-red-500"}`}>
                      {r.last_probe.effect} ({r.last_probe.latency_ms}ms)
                      {r.last_probe.reason && <div className="text-muted-foreground opacity-80">{r.last_probe.reason}</div>}
                    </div>
                  ) : (
                ...
                <td className="px-4 py-3 text-right">
                  <div className="flex flex-col items-end gap-1">
                    <div className="flex items-center gap-2">
                      <button 
                        onClick={() => handleAction(r.id, "Validate", () => PdpRuntimeApi.validate(r.id))}
                        className="px-2 py-1 bg-secondary text-secondary-foreground rounded text-xs hover:opacity-80"
                      >
                        Validate
                      </button>
                      <button 
                        onClick={() => handleAction(r.id, "Probe", () => PdpRuntimeApi.probe(r.id))}
                        className="px-2 py-1 bg-secondary text-secondary-foreground rounded text-xs hover:opacity-80"
                      >
                        Probe
                      </button>
                      <button 
                        onClick={() => handleAction(r.id, "Clear Cache", () => PdpRuntimeApi.clearCache(r.id))}
                    ...
                    {actionStates[r.id] && (
                      <div className="text-[10px] text-muted-foreground max-w-[150px] truncate" title={actionStates[r.id]}>
                        {actionStates[r.id]}
                      </div>
                    )}
                  </div>
