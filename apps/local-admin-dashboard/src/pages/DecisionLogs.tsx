...
const EFFECT_STYLE: Record<DecisionEffect, { cls: string; Icon: typeof ShieldCheck }> = {
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight flex items-center gap-2">
            <Activity className="h-6 w-6 text-primary" /> Audit &amp; Decision Logs
          </h2>
        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y divide-border">
            {loading && events.length === 0 ? (
              <tr><td colSpan={6} className="px-6 py-8 text-center text-muted-foreground">Loading decision logs...</td></tr>
            ) : decisions.length === 0 ? (
              <tr><td colSpan={6} className="px-6 py-8 text-center text-muted-foreground">No decisions recorded yet.</td></tr>
            ) : decisions.map(({ env, d }) => {
              const style = EFFECT_STYLE[d?.decision] ?? EFFECT_STYLE.deny;
              const Icon = style.Icon;
              return (
                <tr key={env.event_id} className="hover:bg-muted/30 transition-colors">
                  <td className="px-6 py-4 text-muted-foreground whitespace-nowrap">
                    {new Date(env.timestamp).toLocaleTimeString()}
                  </td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center gap-1.5 font-medium ${style.cls}`}>
                      <Icon className="h-4 w-4" /> {d?.decision ?? "unknown"}
                    </span>
                  </td>
                  <td className="px-6 py-4 font-mono text-xs">{d?.request_id ?? "-"}</td>
                  <td className="px-6 py-4 text-muted-foreground max-w-xs truncate" title={d?.reason}>{d?.reason ?? "-"}</td>
                  <td className="px-6 py-4 text-xs text-muted-foreground">
                    {d?.matched_policy_ids?.length ? d.matched_policy_ids.join(", ") : "—"}
                  </td>
                  <td className="px-6 py-4 text-right text-muted-foreground">{d?.latency_ms ?? 0}ms</td>
                </tr>
              );
            })}
          </tbody>
